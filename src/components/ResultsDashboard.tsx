...
export const ResultsDashboard = ({
  ...
  return <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <h3 className="text-2xl font-fredoka font-bold">Improvement roadmap</h3>
        <Button variant="outline" onClick={onReset} className="rounded-xl hover-bounce">Analyze another<RotateCcw className="w-4 h-4 mr-2" />
          Analyze Another
        </Button>
      </div>

      <PriorityTabs activePriority={activePriority} onPriorityChange={setActivePriority} counts={counts} />

      <div className="grid gap-4 md:grid-cols-2">
        {filteredImprovements.map((improvement, index) => <ImprovementCard key={improvement.id} improvement={improvement} index={index} screenshot={result.screenshot} />)}
      </div>

      {filteredImprovements.length === 0 && <div className="text-center py-12">
